    ...
    <v-container>
      <v-card>
        ...
        <v-card-text>
          ...
          {{ (page - 1) * itemsPerPage + index + 1 }}
            ...
            <!-- Inline editing untuk PBB Yang Harus Dibayar -->
            <template v-slot:item.PBB_YG_HARUS_DIBAYAR_SPPT="{ item }">
              <v-text-field
                v-if="item.isEditing"
                v-model.number="item.PBB_YG_HARUS_DIBAYAR_SPPT"
                type="number"
                dense
                hide-details
                class="mt-2"
              ...
              ></v-text-field>
              <span v-else>{{ formatCurrency(item.PBB_YG_HARUS_DIBAYAR_SPPT) }}</span>
            </template>
  
            <!-- Inline editing untuk Faktor Pengurang -->
            <template v-slot:item.FAKTOR_PENGURANG_SPPT="{ item }">
              <v-text-field
                v-if="item.isEditing"
                v-model.number="item.FAKTOR_PENGURANG_SPPT"
                type="number"
                step="0.01"
                dense
                hide-details
                ...
                @keyup.esc="cancelEdit(item)"
              ></v-text-field>
              <span v-else>{{ formatCurrency(item.FAKTOR_PENGURANG_SPPT) }}</span>
            </template>
  
            <!-- Inline editing untuk Status Pembayaran -->
            <template v-slot:item.STATUS_PEMBAYARAN_SPPT="{ item }">
              <v-select
                v-if="item.isEditing"
                v-model="item.STATUS_PEMBAYARAN_SPPT"
                :items="keteranganOptions"
                item-title="name"
                item-value="id"
                dense
              ...
              <v-chip 
                v-else
                :color="getStatusColor(item.STATUS_PEMBAYARAN_SPPT)"
                small
                text-color="white"
              >
                {{ getStatusText(item.STATUS_PEMBAYARAN_SPPT) }}
              </v-chip>
            </template>
  
            <!-- Inline editing untuk Nama WP -->
            <template v-slot:item.NM_WP_SPPT="{ item }">
              <v-text-field
                v-if="item.isEditing"
                v-model="item.NM_WP_SPPT"
                dense
                hide-details
                class="mt-2"
                @keyup.enter="saveItem(item)"
                @keyup.esc="cancelEdit(item)"
              ></v-text-field>
              <span v-else>{{ item.NM_WP_SPPT }}</span>
            </template>
            ...
            <!-- Status indicator untuk editing -->
            <template v-slot:item.editStatus="{ item }">
              <v-chip
                v-if="item.isEditing || item.isNew"
                :color="item.isNew ? 'blue' : 'orange'"
                small
                text-color="white"
              >
                {{ item.isNew ? 'Baru' : 'Edit' }}
              </v-chip>
            </template>
          </v-data-table>
        </v-card-text>
      ...
      <v-dialog v-model="deleteDialog" max-width="500px">
        <v-card>
          ...
          <v-card-text>
            Apakah Anda yakin ingin menghapus data SPPT ini?
            <br>
            <strong>{{ itemToDelete ? itemToDelete.NOP : '' }}</strong>
            <br>
            <strong>{{ itemToDelete ? itemToDelete.NM_WP_SPPT : '' }}</strong>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
