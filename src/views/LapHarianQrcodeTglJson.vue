    ...
    <v-container fluid>
      <v-card class="elevation-2 rounded-lg">
        ...
        >
          ...
          <!-- Format Kolom Status Pembayaran yang Diperbarui -->
          <template v-slot:item.status="{ item }">
            <v-chip
              :color="(item.targetTagihan > 0 && item.totalIuran >= item.targetTagihan) ? 'success' : 'warning'"
              size="small"
              class="text-uppercase font-weight-bold"
            >
              {{ (item.targetTagihan > 0 && item.totalIuran >= item.targetTagihan) ? 'LUNAS' : 'BELUM LUNAS' }}
            </v-chip>
          </template>
...
// Pastikan variabel penampung data transaksi ini sudah ada di <script setup> Anda:
 const historiList = ref([]);
  ...
  /*
// Tambahkan fungsi ini di dalam <script setup> Anda
const hitungTotalBayar = (nopWarga) => {
  // Pastikan Anda memiliki sumber data histori transaksi, misalnya allTransactions atau historiList
  if (!allTransactions.value || allTransactions.value.length === 0) return 0;

  // Filter transaksi berdasarkan NOP dan jumlahkan nominal bayarnya
  const matchedTransactions = allTransactions.value.filter(item => {
    return String(item.NOP).trim() === String(nopWarga).trim();
  });

  // Jumlahkan total uang yang sudah dibayar
  let total = 0;
  matchedTransactions.forEach(trx => {
    total += Number(trx.jumlah || trx.bayar || 0);
  });
...
// Di dalam proses fetchLaporanHistori, saat membentuk rekap:
wargaList.value.forEach(pembayaran => {
  // Cari data target tagihan dari JSON berdasarkan ID atau NOP warga
  const targetFromJson = originalJsonData.value.find(j => j.NOP === pembayaran.id || j.NOP === pembayaran.nop);
  
  // Tentukan target nominal (jika JSON tidak ada, fallback ke nilai default misal 15000)
  const nominalTarget = targetFromJson ? Number(String(targetFromJson.PBB_YG_HARUS_DIBAYAR_SPPT || targetFromJson.target || 15000).replace(/[^0-9]/g, '')) : 15000;

  rekap[pembayaran.id] = {
    NOP: pembayaran.id,
    namaWarga: pembayaran.nama,
    totalIuran: 0,
    targetTagihan: nominalTarget, // Simpan target tagihan per warga
    histori: []
  };
});
...
//data awalnya ini
    wargaList.value.forEach(warga => {
      // Cari data di JSON berdasarkan NOP (pastikan trim dan string)
      const targetFromJson = originalJsonData.value.find(j => {
        //const jsonNop = j.NOP || j.nop || j.NOP;
        const jsonNop = j.NOP;
        return jsonNop && String(jsonNop).trim() === String(warga.id).trim();
      });
      
      // Ambil nilai tagihan dari JSON. Jika JSON tidak ada/kosong, set 0.
      let nominalTarget = 0;
      if (targetFromJson) {
      //const rawTagihan = targetFromJson.PBB_YG_HARUS_DIBAYAR_SPPT || targetFromJson.target || 0;
      const rawTagihan = targetFromJson.PBB_YG_HARUS_DIBAYAR_SPPT || targetFromJson.target || targetFromJson.jumlah_tagihan || 0;
...
wargaList.value.forEach(warga => {
  // 2. Gunakan cleanNop pada kedua sisi agar pencocokan akurat 100% 
  // (mengabaikan perbedaan titik, strip, atau spasi)
  const targetFromJson = originalJsonData.value.find(j => {
    const jsonNop = j.NOP;
...
//                       targetFromJson.target || 
  //                     targetFromJson.jumlah_tagihan || 
   //                    targetFromJson.tagihan || 0;

    nominalTarget = Number(String(rawTagihan).replace(/[^0-9]/g, '')) || 0;
  }

// 3. Ambil total iuran yang sudah dibayar dari Firestore / histori lokal
  const totalSudahBayar = hitungTotalBayar(warga.id); 

  // 4. Tentukan status secara objektif
  const statusBayar = totalSudahBayar >= nominalTarget ? 'LUNAS' : 'BELUM LUNAS';


  rekapMap[warga.id] = {
    NOP: warga.id,
    namaWarga: warga.nama,
    tagihanTotal: nominalTarget, // <--- Murni dari PBB_YG_HARUS_DIBAYAR_SPPT JSON
    targetTagihan: nominalTarget, // <--- Murni dari PBB_YG_HARUS_DIBAYAR_SPPT JSON
    totalIuran: totalSudahBayar,   // Total bayar dari Firestore (bisa beda/lebih kecil)
    status: statusBayar,
    // Contoh link QR yang dikirim ke halaman verifikasi:
    qrUrl: `http://192.168.4.140:5199/verify-kuitansi-pbb?NOP=${warga.id}&start=${startDate}&end=${endDate}&status=${statusBayar}&bayar=${totalSudahBayar}&target=${nominalTarget}`,
    histori: []
  };
});
    ...
    querySnapshot.docs.forEach(doc => {
      ...
      // Jika ada warga bayar tapi belum terdaftar di list master
      if (!rekapMap[NOP]) {
        const targetFromJson = originalJsonData.value.find(j => {
        //  const jsonNop = j.NOP || j.nop || j.NOP_SPPT;
          const jsonNop = j.NOP;
...
const bodyData = rekapHistori.value.map((item, index) => {
 ...
 const targetTagihan = item.tagihanTotal || 0; 
  ...
  // Status akan LUNAS jika pembayaran sudah mencapai/melewati target tagihan (0 atau nominal asli)
 // const statusPembayaran = sudahDibayar >= targetTagihan ? 'LUNAS' : 'BELUM LUNAS';
 
 // Status LUNAS hanya jika target tagihan ada (> 0) dan sudah dibayar lunas
...
//  const qrTextContent = `https://iuran-warga-five.vercel.app/verify-kuitansi?nop=${item.NOP}&start=${startDate.value}&end=${endDate.value}`;
...
let statusVerifikasi = 'BELUM LUNAS';

// Validasi ketat: Target harus > 0 DAN pembayaran harus mencukupi
if (targetTagihan > 0 && sudahDibayar >= targetTagihan) {
    statusVerifikasi = 'LUNAS';
} else {
    statusVerifikasi = 'BELUM LUNAS';
}
  
  // GUNAKAN SYARAT YANG SAMA: Harus ada target tagihan (>0) dan sudah dibayar lunas
  const statusPembayaran = (targetTagihan > 0 && sudahDibayar >= targetTagihan) ? 'LUNAS' : 'BELUM LUNAS';
 
  // URL untuk verifikasi online
  // PERBARUI URL QR CODE: Sisipkan parameter status, total, dan target agar halaman verifikasi membaca data yang sama
//  const qrTextContent = `http://192.168.4.140:5199/verify-kuitansi-pbb?NOP=${item.NOP}&start=${startDate.value}&end=${endDate.value}&status=${statusPembayaran}&bayar=${sudahDibayar}&target=${targetTagihan}`;
  ...
  return {
    ...
    namaWarga: item.namaWarga,
    transaksi: `${item.histori.length} kali`,
    status: statusPembayaran, // Status sudah konsisten
    jumlah: formatRupiah(item.totalIuran),
    qrcode: qrBase64 
  };
});
